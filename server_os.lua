local OSNAME, VERSION = "AuroraOS", "5.0"
...
local function biosMenu()
  local items={
    {label="Boot AuroraOS", act=function() return "aurora" end},
    {label="Boot CraftOS", act=function() shell.run("shell"); os.reboot() end},
    {label="Boot other file", act=function() bootOther(); os.reboot() end},
    {label="Recovery (no password)", act=function() recoveryShell(); os.reboot() end},
    {label="Settings (delay, default)", act=function() biosSettings() end},
    {label="Monitors", act=function() local m=gatherMonitors(); if #m>=1 then term.redirect(term.native()); W,H=term.getSize(); pickMonitors(m) end end},
    {label="Install Server OS", act=function() installServerOS() end},
    {label="Power off", act=function() os.shutdown() end},
  }
  local sel=1
  while true do
    term.redirect(term.native()); applyTheme()
    term.setBackgroundColor(colors.blue); term.clear(); term.setCursorPos(1,1)
    local bw=math.min(W-4,38); local bh=#items+5; local bx=math.floor((W-bw)/2)+1; local by=math.floor((H-bh)/2)+1
    ...
    term.setCursorPos(bx+2,by+2); term.write("Up/Down: select   Enter: go")
    for i,it in ipairs(items) do
      local y=by+3+i
      if i==sel then
        term.setBackgroundColor(colors.cyan); term.setTextColor(colors.black)
        term.setCursorPos(bx+2,y); term.write("> "..it.label..string.rep(" ", math.max(0, bw-5-#it.label)))
      else
        term.setBackgroundColor(colors.lightGray); term.setTextColor(colors.black)
        term.setCursorPos(bx+2,y); term.write("  "..it.label)
      end
    end
    term.setTextColor(colors.blue); term.setBackgroundColor(colors.lightGray)
    term.setCursorPos(bx+2,by+bh-1); term.write("Item "..sel.."/"..#items)
    local ev,p1=os.pullEvent()
    if ev=="key" then
      if p1==keys.up then sel=sel-1; if sel<1 then sel=#items end
      elseif p1==keys.down then sel=sel+1; if sel>#items then sel=1 end
      elseif p1==keys.enter then local r=items[sel].act(); if r=="aurora" then return true end
      elseif p1==keys.one then local r=items[1].act(); if r=="aurora" then return true end
      elseif p1==keys.two then items[2].act()
      elseif p1==keys.three then items[3].act()
      elseif p1==keys.four then items[4].act()
      elseif p1==keys.five then items[5].act()
      elseif p1==keys.six then items[6].act()
      elseif p1==keys.seven then items[7].act()
      elseif p1==keys.q then items[8].act() end
    end
  end
end
